...
proc sql;
	select avg(droprate) as Average,	
		std(droprate) as SD
	from analytical_merged
	where gender = 'F'
	;run;
...
proc sql;* levels are ordinal;
create table grade_levels as 
	select avg(drop7) as Seventh,
		avg(drop8) as Eighth,
		avg(drop9) as Ninth,
		avg(drop10) as Tenth,
...
proc transpose data = grade_levels
               out = grade_levels;
run;
...
*from using the analytical file, the same analysis is done;
data analytical_merged;
   set analytical_merged;
   droprate = DTOT / ETOT;
run;
